...
local Window = library:CreateWindow("InFLames Hub", "The Survival Game [BETA]", 10044538000)
...
PlantToggle = FarmingFrame:CreateToggle("Plant not working", "Sets the farming mode to Plant", function(Bool)
    AutoPickup = Bool
...
TeleportsFrame:CreateBox("animal", 10044538000, function(Text)
    local Animal = GetAnimal(Text)

    if Animal ~= nil then
        if Animal.HumanoidRootPart then
            BoatTeleport(Animal.HumanoidRootPart.Position)
        end
    end
end)
